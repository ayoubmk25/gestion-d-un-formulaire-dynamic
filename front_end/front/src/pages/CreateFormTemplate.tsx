...
const CreateFormTemplate = () => {
  ...
  const { createFormTemplate } = useApi();
    ...
    setFormState({
      ...
      fields: [
        ...formState.fields,
        {
          ...
          isGrouped: false,
          groupName: '',
          numberType: 'float' // Add default number type for new fields
        }
      ]
    });
  };
  ...
  const addOption = (fieldIndex) => {
    ...
    field.options.push({
      label: `Option ${field.options.length + 1}`, // Dynamic label, keeping "Option"
  ...
  const validateForm = () => {
    ...
    for (let i = 0; i < formState.fields.length; i++) {
      ...
      if (!field.label.trim()) {
        toast({
          title: "Erreur de validation", // Translated
          description: `Le champ n°${i + 1} n'a pas d'étiquette`, // Translated (with variable)
          variant: "destructive",
        });
        return false;
      }
      ...
      if ((field.type === 'select' || field.type === 'radio' || 
        ...
        toast({
          title: "Erreur de validation", // Translated
          description: `Le champ "${field.label}" doit avoir au moins une option`, // Translated (with variable)
          variant: "destructive",
        });
        return false;
      }
      ...
      if ((field.type === 'radio' || field.type === 'checkbox') && 
        ...
        toast({
          title: "Erreur de validation", // Translated
          description: `Le champ groupé "${field.label}" doit avoir un nom de groupe`, // Translated (with variable)
          variant: "destructive",
        });
        return false;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Call API to create form template
      ...
      toast({
        title: "Modèle de formulaire créé", // Translated
        description: "Votre modèle de formulaire a été créé avec succès", // Translated
        variant: "default",
      });
      ...
      toast({
        title: "Erreur lors de la création du modèle", // Translated
        description: "Un problème est survenu lors de la création de votre modèle de formulaire", // Translated
        variant: "destructive",
      });
    } finally {
    ...
    <Layout>
      <div className="container py-8 animate-fade-in">
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-2xl font-bold">Créer un modèle de formulaire</h1> {/* Translated */}
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Informations de base sur votre modèle de formulaire</CardDescription> {/* Translated */}
              </CardHeader>
                    ...
                    <div className="space-y-4">
                            ...
                            {field.options && field.options.map((option, optionIndex) => (
                              <div key={optionIndex} className="flex items-center space-x-2">
                                <Input
                                  ...
                                  placeholder={`Option ${optionIndex + 1}`} // Keeping Option + number
                                  className="flex-1"
                                />
                                <Button
                                  variant="ghost"
                      ...
                      {field.type === 'number' && (
                        <div className="space-y-2">
                          ...
                          <Select
                            value={field.numberType || 'float'} // Default to float if not set
